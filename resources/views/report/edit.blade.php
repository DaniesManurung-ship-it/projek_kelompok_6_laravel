    ...
    <div class="row mb-4">
        <div class="col">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h1 class="fw-bold">
                        <i class="fas fa-edit me-2 text-primary"></i>Edit Laporan
                    </h1>
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{ route('report.index') }}">Laporan</a></li>
                            <li class="breadcrumb-item"><a href="{{ route('report.show', $report->id) }}">{{ $report->title }}</a></li>
                            <li class="breadcrumb-item active">Edit</li>
                        </ol>
                    </nav>
                </div>
                <a href="{{ route('report.show', $report->id) }}" class="btn btn-outline-secondary">
                ...
                <div class="card-body p-4">
                    <form action="{{ route('report.update', $report->id) }}" method="POST">
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="type" class="form-select @error('type') is-invalid @enderror" required>
                                    ...
                                    @foreach($types as $type)
                                        <option value="{{ $type }}" {{ old('type', $report->type) == $type ? 'selected' : '' }}>
                                            {{ $type }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="status" class="form-select @error('status') is-invalid @enderror" required>
                                    ...
                                    @foreach($statuses as $status)
                                        <option value="{{ $status }}" {{ old('status', $report->status) == $status ? 'selected' : '' }}>
                                            {{ $status }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="format" class="form-select @error('format') is-invalid @enderror" required>
                                    ...
                                    @foreach($formats as $format)
                                        <option value="{{ $format }}" {{ old('format', $report->format) == $format ? 'selected' : '' }}>
                                            {{ $format }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="file_path" class="form-control @error('file_path') is-invalid @enderror" 
                                       value="{{ old('file_path', $report->file_path) }}">
                                @error('file_path')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="d-flex justify-content-between mt-4">
                            <div>
                                <a href="{{ route('report.show', $report->id) }}" class="btn btn-secondary">
                            ...
                            <div>
                                <form action="{{ route('report.destroy', $report->id) }}" 
                                    ...
                                    <button type="button" 
                                            ...
                                            onclick="if(confirm('Hapus laporan ini?')) this.form.submit()">
